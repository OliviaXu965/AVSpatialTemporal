...
out1FileName = ['PointingTest_sub' num2str(ExpInfo.subjID)];
...
KbWait(-1); %-1 if using the keyboard; -3 if using the attached keypad
WaitSecs(1);
Screen('Flip',windowPtr);
...
for i = 1:ExpInfo.numTrials 
    ...
    [data(2,i), data(4,i), data(5,:)] = PresentVisualStimulus(data(3,i),ScreenInfo, ...
        ExpInfo, windowPtr);
end
ShowCursor;
