...
try
    ...
    out1FileName = ['TOJ_sub', num2str(ExpInfo.subjID)]; %create file name
    ...
    ExpInfo.numTrials        = 25;
    
    % number of trials in each block to decide break
    ExpInfo.numTrialsinBlock = 100;
    ...
    trialOrder_temp          = 1:ExpInfo.numSOA;
    for ii = 1:ExpInfo.numTrials
        idx_start = (ii-1)*ExpInfo.numSOA + 1;
        idx_end   = ii*ExpInfo.numSOA;
        shuffle_idx = randperm(ExpInfo.numSOA, ExpInfo.numSOA);
        ExpInfo.trialOrder(idx_start:idx_end) = trialOrder_temp(shuffle_idx);
    end
